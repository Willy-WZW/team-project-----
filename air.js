...
const apiValue = document.querySelector("#apiValue");
...
fetch(
  ...
  .then((data) => {
    ...
    groupData = areaArr.reduce((acc, element) => {
      ...
      let dateKey = `${measureDate.getFullYear()}-${(measureDate.getMonth() + 1).toString().padStart(2,"0")}-${measureDate.getDate().toString().padStart(2,"0")} ${measureDate.getHours().toString().padStart(2,"0")}:${measureDate.getMinutes().toString().padStart(2,"0")}`;
      //若累加器中沒有該城市名，則初始化累加器
      if (!acc[countyName]) {
    ...
    for (let countyName in groupData) {
      ...
      for (let dateKey in groupData[countyName]) {
        ...
        let avgSo2 = Math.round(sumSo2 / countSo2);
        let avgNo2 = sumNo2 / countNo2;
